    ...
    >
      <div class="max-w-4xl mx-auto px-6">
        <h2 class="text-3xl md:text-4xl font-bold font-display text-center mb-16 text-text">
          <span class="text-accent-secondary">&gt;</span> {sectionName()}
        </h2>
        ...
        <div class="relative">
          ...
          <For each={experiences()}>
            {(exp, index) => (
              ...
              >
                {/* Dot */}
                  <div class="absolute left-4 md:left-1/2 w-3 h-3 rounded-full bg-accent-primary border-2 border-bg -translate-x-1/2 mt-2 shadow-glow-primary" />
                        ...
                        <For each={exp.highlights}>
                          {(item) => (
                            <li class="text-xs text-text-secondary leading-relaxed flex items-start gap-1.5">
                              <span class="text-accent-primary mt-0.5 shrink-0">▹</span>
                              <span>{item}</span>
                            </li>
                          )}
                        </For>
